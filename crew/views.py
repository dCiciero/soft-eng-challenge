...
# from crew.serializers import CrewMemberSerializer
# from . import services
# from ship import services as ss


# crew_members = services.get_queryset()
# @api_view(["GET", "POST"])
# def list_crew(request):
#     if request.method == "GET":
#         if crew_members:
#             serialized_crew = CrewMemberSerializer(crew_members, many=True)
...
#         serialized_record = CrewMemberSerializer(data=request.data)
...
#     crew_member = services.get_queryset(id)
#     if request.method == "GET":
#         if crew_member:
#             if isinstance(crew_member, Crew_Member):
#                 crew_serializer = CrewMemberSerializer(crew_member, many=False)
#                 return Response(crew_serializer.data)
#         return Response({"Error":f"No ship with Id={id}"}, status=status.HTTP_400_BAD_REQUEST)
    
#     elif request.method == "PUT":
#         print(request.data)
#         print(crew_member.ship_assigned.pk)
#         # serialized_record = CrewMemberSerializer(data=crew_member)
#         # if serialized_record.is_valid():
#         #     print(f"Hehehe{serialized_record.validated_data.get('name')}")
...
#         serialized_record = CrewMemberSerializer(crew_member, data=data)
#         if serialized_record.is_valid():
#             print(serialized_record.validated_data.get('ship_assigned'))
#             # serialized_record.validated_data['ship_assigned'] = to_ship
#             try:
#                 serialized_record.save()
#                 # crew_member.delete()
